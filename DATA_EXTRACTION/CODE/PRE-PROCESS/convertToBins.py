...
def SplitIntoBins(df, flag = False):
	'''
	...
	while index < df.shape[0]:
		...
		index += 1
		while index < df.shape[0] and df.loc[index]['tweet_time'] - time <= 300:
			row = df.loc[index]
			list_ = []
			list_.append(row['tweet_time'])
			list_.append(row['tweet_text'])
		...
		if label == 'O' or flag == True:
			event_id = str(-1)
		else:
			event_id = str(labels[label])

		str_ += "'," + " 'event_type': '" + label + "', 'event_id': " + event_id + ", 'doc': '" + row['event'] + "'}"
